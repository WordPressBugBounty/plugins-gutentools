            ...
            foreach ( $this->blocks as $block ) {
                ...
                foreach ( self::$devices as $device ) {
                    $devices_style = [
                        ...
                        '.gutentools-next-arrow' => array_merge( $arrowPositionRight[ $device ] ),
                        '.gutentools-prev-arrow' => array_merge( $arrowPositionLeft[ $device ] ),
                        '.slick-dots li button'  => array_merge( $dotsSize[ $device ], $dotsRadius[ $device ] ),
                    ];

                    self::add_styles( [ 'attrs' => $attrs, 'css' => $devices_style ], $device );
                }
                ...
                self::add_styles( [ 'attrs' => $attrs, 'css' => $desktop_css ] );
                self::add_styles( [ 'attrs' => $attrs, 'css' => $hover_css ], 'desktop', true );
                ...
                ?>
                var slider = {
                    slidesToShow  : <?php echo (int) $slide; ?>,
                    slidesToScroll: 1,
                    infinite      : true,
                    ...
                    responsive: [
                        {
                            breakpoint: 768,
                            settings: { slidesToShow: <?php echo (int) $slideToShow['values']['mobile']; ?> }
                        },
                        {
                            breakpoint: 1025,
                            settings: { slidesToShow: <?php echo (int) $slideToShow['values']['tablet']; ?> }
                        }
                    ]
                };
                jQuery(<?php echo wp_json_encode( $selector ); ?>).slick( slider );
                <?php
                self::add_scripts( ob_get_clean() );
            }
        }
