...
class Gutentools_CSS {
	...
	public function add_properties( array $properties ) {
	    foreach ( $properties as $property => $value ) {
	        ...
	        if ( $property === 'border' || strpos( $property, 'border-' ) === 0 ) {
	            $this->handle_border_property( $property, $value );
	        } else {
	...
	public function css_output()
	{
	 ...
	 protected function handle_border_property( $property, $value ) {
        // Handling case where 'border' is an array of top, right, bottom, left
        if ( is_array( $value ) ) {
        	if( isset( $value[ 'left' ] ) || isset( $value[ 'right' ] ) || isset( $value[ 'top' ] ) || isset( $value[ 'bottom' ] ) )  {
	            foreach ( $value as $side => $border_values ) {
	                $border_width = !empty( $border_values[ 'width' ] ) ? $border_values[ 'width' ] : 'initial';
	                $border_style = !empty( $border_values[ 'style' ] ) ? $border_values[ 'style' ] : 'solid';
	                $border_color = !empty( $border_values[ 'color' ]) ? $border_values[ 'color' ] : 'black';

	                // Add individual border sides like 'border-top', 'border-right', etc.
	                $this->add_rule( "border-{$side}", "$border_width $border_style $border_color" );
	            }
	        }else{
	        	$border_width = !empty( $value[ 'width' ] ) ? $value[ 'width' ] : 'initial';
				$border_style = !empty( $value[ 'style' ] ) ? $value[ 'style' ] : 'solid';
				$border_color = !empty( $value[ 'color' ] ) ? $value[ 'color' ] : 'white';
				
				$this->add_rule( "border", "$border_width $border_style $border_color" );
	        }

        } else {
            // Add general border property like 'border'
            $this->add_rule($property, $value);
        }
    }
